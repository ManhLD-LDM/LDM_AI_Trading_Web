    ...
    def predict(self, history_data: np.ndarray, model_type: str = "lstm") -> dict:
        ...
        try:
            if model_type == "xgboost":
                # XGBoost expects 50 candles flattened
                input_data = np.array(history_data[-50:]).reshape(1, -1)
                ...
                if len(history_data) >= 512:
                    input_data = np.array(history_data[-512:]).astype(np.float32)
                else:
                    # Pad with zeros if not enough data
                    input_data = np.zeros((512, 5), dtype=np.float32)
                    input_data[-len(history_data):] = np.array(history_data).astype(np.float32)
