...
export const TradingAPI = {
  ...
  // Paper Trading
  executePaperTrade: (payload: { symbol: string; action: string; price: number; quantity: number }, token: string) =>
    apiPost('/api/paper/execute', payload, token),

  getPortfolio: (token: string) => apiGet('/api/paper/portfolio', token),

  getTradeHistory: (token: string, page = 1, pageSize = 20, symbol?: string) => {
    const qs = new URLSearchParams({ page: String(page), page_size: String(pageSize) });
    if (symbol) qs.set('symbol', symbol);
    return apiGet(`/api/paper/history?${qs}`, token);
  },
